...
static void inventory_handler_cb(void *arg, uint32_t device_class_instance, bool is_available)
{
    switch (DDMP2_INVENTORY_CLASS(device_class_instance))
    {
    case SDP0:
        //fallthrough
    case SBMEB0:
        if (is_available)
        {
            ...
            for (uint8_t db_idx = 0; db_idx < conn_fanmot_db_elements; db_idx++)
            {
...
static int initialize_connector_fan_motor(void)
{
    ...
    {
        LOG(E, "Queue creation failed for invent control task");
    }
...
static void install_parameters(void)
{
    uint8_t index;
    int instance;
    uint32_t device = MTR0;
     // Send the request for register FAN1, FAN2 and Motor devices instance to broker
    for (index = 0; index < MAX_NUM_DEVICE; index++)
    {
        device = MTR0;
        instance = broker_register_instance(&device, connector_pwm_fan_motor.connector_id);
        if (instance < 0)
        {
            LOG(E, "Failed to allocate instanc for MTR0");
        }
    }
    ...
    for (index = 0; index < IAQ_RANGE_LEVELS; index++)
    {
        ...
        if (instance < 0)
        {
            LOG(E, "Failed to allocate instanc for IVAQR0");
        }
    }
}
...
static void conn_fan_motor_process_task(void *pvParameter)
{
	...
	inventory_handler_start(&l_ih, &connector_pwm_fan_motor);
            ...
            if (true == ptr_ctrl_algo->per_tmr_exp)
            {
                ...
                if (ptr_ctrl_algo->min_counter >= 1)    //FAN_MOTOR_VALIDATION_INTERVAL_MIN )
                {
                    ...
                    for (invent_device_id_t dev_id = 0; dev_id < MAX_NUM_DEVICE; dev_id++)
                    {
                        ...
                        LOG(I, "dev%d set_rpm = %d tacho = %d min_exp_rpm = %d", dev_id, ptr_ctrl_algo->set_rpm[dev_id],
                                ptr_ctrl_algo->dev_tacho[dev_id], min_limit_rpm);
